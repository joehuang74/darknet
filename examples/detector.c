...
void label_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
...
void run_detector(int argc, char **argv)
{
    ...
    if(argc < 4){
        fprintf(stderr, "usage: %s %s [train/test/valid/label] [data cfg] [cfg] [weights (optional)]\n", argv[0], argv[1]);
        return;
    }
    ...
    else if(0==strcmp(argv[2], "label")) label_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, fullscreen);
